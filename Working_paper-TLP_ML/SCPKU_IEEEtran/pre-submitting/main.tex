...
\usepackage{multirow}
\usepackage{array}
\usepackage{placeins} % 放在导言区
\usepackage[T1]{fontenc}
\usepackage{lmodern} % 类似 Computer Modern 的扩展版（不完全等于 CMU）
\usepackage[font=footnotesize]{caption}
\captionsetup[table]{skip=4pt}   % 表格caption与表格的距离
\captionsetup[figure]{skip=4pt}  % 图caption与图的距离
...
% ---- Bibliography ----
% IEEEtran does not use natbib by default; keep \cite{} usage unchanged.
% If you want natbib-style citation formats later, you can add:
% \usepackage[numbers]{natbib}

% ---- Table spacing ----
\renewcommand{\arraystretch}{1.1} % 行高放大
\setlength{\tabcolsep}{4pt}        % 列间距（默认 6pt）
...
% ===================== PDF Metadata =====================
\pdfinfo{
/Title (Stable Long-Tail Regression with MoE on CivitasGrid-TLP: A Dataset and Benchmark for Urban Dynamics)
/Author (Winston Huang)
}

\begin{document}
% best marker that does NOT change column width/alignment
\newcommand{\bst}{\rlap{$^{*}$}}



% ===================== TITLE + AUTHOR =====================
\title{RMSE-Anchored Contrastive Calibration for Reliable Long-Tail Regression on Urban Proxy Dynamics}

\author{
\IEEEauthorblockN{Winston Huang
\IEEEauthorblockA{
Olive Tree International Academy, BFSU\\
Hangzhou, China\\
cnhuangwenqin@gmail.com}
}
}
...
% ===================== ABSTRACT =====================
\begin{abstract}
Urban proxy dynamics exhibit heavy-tailed distributions: most grids remain stable, while a small fraction of shocks dominate risk. Using CivitasGrid-TLP (2015–2024), we benchmark tabular regressors for grid-level prediction of $\Delta$WorldPop and $\Delta$VIIRS under GroupSplit(\texttt{grid\_id}). A two-expert Mixture-of-Experts (MoE) proves ineffective in this setting: tail routing is weakly identifiable, boundary decisions are unstable, and misrouting combined with inverse signed-log transformations amplifies errors, degrading both overall and tail metrics compared to a single XGBoost baseline. In contrast, a model-agnostic post-hoc affine calibrator trained with an RMSE-anchored contrastive objective consistently improves tail reliability while preserving overall fit across diverse baselines. The key insight is that in long-tail urban regression, reliable tail gains require anchoring—otherwise routing and calibration collapse.
\end{abstract}
...
\subsection{Economic Dynamics Prediction: Why This Matters}
Transportation planning has become increasingly important, as effective transportation systems can boost local economies and enhance urban sustainability. In this context, developing tools that enable city planners to evaluate the potential economic outcomes of transportation infrastructure investments is critical. However, forecasting these economic outcomes remains challenging in practice. Economic changes result from complex interactions among regional conditions, municipal finances, and mobility patterns, making them difficult to predict with traditional modeling approaches. Machine learning models, which can capture complex nonlinear relationships, offer a promising alternative for economic dynamics modeling, though significant challenges remain.

\subsection{The Challenge of Long-tail Urban Socioeconomic Dynamics Prediction}

In this study, we construct and benchmark machine learning models to predict year-to-year changes in proxy urban economic signals (WorldPop and VIIRS) at the grid level.

We observe that differenced proxy signals follow a long-tailed distribution: most values cluster near zero, while a small but substantial number of grid cells experience abrupt shocks, placing them far from the bulk of the distribution. These outliers are driven by mechanistic factors that frequently occur in practice, rather than simple measurement noise, making it essential for models to reliably predict tail-grid outcomes. However, because most conventional machine learning models optimize for average error, they typically underperform when predicting tail dynamics. Heavy-tailed distributions can also cause training instabilities, degrade robustness under extreme events, and lead to larger-than-expected worst-case errors.

Large datasets are required for all machine learning models. However, official economic statistics are typically insufficient at fine spatial resolution, expensive to collect, temporally delayed, and inconsistent across regions. This motivates the use of scalable proxy signals for urban economic dynamics modeling.

\subsection{Research Questions and Overview}

These challenges motivate a core research question: how can we build a scalable and reliable machine learning framework for forecasting proxy-based urban economic dynamics under long-tailed targets and diverse regional conditions? To address this, we construct a grid-level dataset that integrates transportation network features with proxy economic signals, and we benchmark several strong baseline models under appropriate evaluation settings. We further investigate modeling strategies that optimize tail and worst-case performance, aiming to provide practical and robust tools for forecasting transportation infrastructure-driven economic outcomes.

\subsection{Our Contributions}
\begin{itemize}
    \item \textbf{Dataset:} We construct a grid-level panel dataset integrating transportation network features with proxy economic signals (WorldPop / VIIRS), enabling scalable urban economic dynamics prediction.
    
    \item \textbf{Benchmark:} We provide a benchmark evaluation protocol and compare multiple strong baseline models under appropriate generalization settings for long-tail dynamics prediction.

    \item \textbf{Long-tail modeling:} We investigate modeling strategies that improve tail and worst-case performance under heavy-tailed targets, addressing the limitations of average-error-optimized regression models.

    \item \textbf{Practical relevance:} We frame the task as an infrastructure-driven economic outcome evaluation problem, aiming to support robust and practical decision tools for transportation planning.
    
\end{itemize}

Next, we review prior work on long-tail regression, mixture-of-experts learning under heterogeneity, and proxy-based socioeconomic indicators, which together motivate our benchmark setting.
...
In summary, prior work validates the use of population and nighttime luminosity as scalable economic proxies and extensively explores MoE for learning under heterogeneity. Long-tail regression research shows that standard MSE/MAE training biases models toward head samples, and remedies such as Balanced MSE and distribution smoothing (LDS/FDS) can improve tail performance under heavy-tailed targets. However, existing studies rarely provide unified benchmarks for grid-level proxy dynamics prediction integrating transportation morphology features, long-tailed targets, and robust evaluation. To address this gap, we introduce CivitasGrid-TLP as a benchmark dataset and investigate robust long-tail modeling strategies with controlled MoE and contrastive learning frameworks.
...
CivitasGrid-TLP is a grid-level panel dataset designed for proxy-based urban economic dynamics prediction~\cite{huang_2026_18144416}. It integrates transportation network features and remotely sensed proxy economic signals (WorldPop population density and VIIRS nighttime light) across thousands of city grids (30\" -60\" resolution).
...
As shown in Table~\ref{tab:datasources}, we extracted transportation features from OpenStreetMap and linked them with proxy economic signals including VIIRS nighttime light intensity and WorldPop population density. Unlike existing work, which typically focuses on coarse administrative regions, CivitasGrid-TLP provides a grid-based benchmark at kilometer-level resolution, supporting robust evaluation across regions and countries.
...
Differencing captures local economic dynamics and reduces baseline differences across regions. Most grid cells experience minor year-to-year fluctuations, while a non-negligible subset undergoes abrupt shocks. This long-tailed characteristic motivates strategies prioritizing tail and worst-case performance.
...
% =====================================================
\section{Methodology}
This section introduces the benchmark baselines and our controlled experimental variants, including an objective-level contrastive learning (CL) calibration layer and a two-expert MoE design for improving tail and worst-case performance.

\subsection{Problem Setup and Tail Definition}
\label{sec:problem_setup}

We evaluate two strategies for improving tail performance—Mixture-of-Experts and contrastive learning calibration—on the CivitasGrid-TLP dataset. We consider a grid-level regression problem where each sample has features $x_i$ representing year-to-year changes in proxy socioeconomic signals (WorldPop and VIIRS). The targets exhibit long-tailed distributions, with most samples near zero and a small but significant fraction at extreme values. We define tail samples as those in the lower 5\% or upper 5\% of the empirical distribution:
\begin{equation}
\begin{aligned}
...
In this section we present and analyze experimental results across two targets and two splitting strategies. We find that contrastive learning (CL) calibration substantially improves tail performance, while the two-expert MoE approach underperforms the baseline.
...
First, predictions of $\Delta$VIIRS (nighttime light change) are consistently weak across all baselines. Under random and group splits, $R^2$ values remain near or below zero, indicating minimal predictive signal beyond a mean baseline. This likely reflects noise amplification from the differencing procedure on an already noisy proxy. Thus, $\Delta$WorldPop is treated as the primary target for evaluation.

Table~\ref{tab:bench_dworldpop} shows $\Delta$WorldPop benchmark performance under both Random Split (RS) and GroupSplit (GS) protocols, reporting overall ($R^2$, RMSE) and tail-specific ($R^2_{\text{tail}}$, RMSE$_{\text{tail}}$) metrics. Under RS, tree-based models (ExtraTrees, XGBoost) achieve the strongest performance ($R^2 \approx 0.84$, RMSE $\approx 21$--22), while MLP performs competitively and linear models lag. Under GS, all models degrade, reflecting increased difficulty in spatial generalization via grid\_id splitting. Consistently, tail errors substantially exceed overall errors across all models.

Since RS may allow information leakage across years within the same grid\_id, we treat \textbf{GS results as the primary reference} for subsequent comparisons. These benchmarks motivate focusing on tail robustness improvement under GS conditions.


\subsection{Contrastive Learning Calibration: Main Results}

% Requires: \usepackage{booktabs,multirow}
% --- Wide table (spans two columns) split into stacked blocks by model ---
% --- add once, before the first tabular* ---
\FloatBarrier
\begin{table*}[!t]
\centering
\scriptsize
...
% NOTE: The uploaded workbook does not contain XGBoost rows, so the XGBoost block is intentionally omitted here.
\vspace{2ex}
% XGBoost
\begin{tabular*}{0.925\textwidth}{@{\extracolsep{\fill}}ll rrr @{\hspace{4pt}} rrr @{\hspace{4pt}} rrr @{\hspace{4pt}} rrr@{}}
\toprule
...
We present and analyze Contrastive Learning-adjusted models compared to benchmarks. We compare Base (no calibration), Mix (RMSE+CL), and CL-only variants while keeping the underlying predictor fixed. Both CL and RMSE+CL substantially improve tail performance over Base, but RMSE+CL maintains superior overall accuracy.
...
Table~\ref{tab:cl_wide_blocks} reveals that CL calibration strongly depends on the objective. The mixed objective (RMSE+CL) provides consistent improvements, while CL-only is often unstable and substantially degrades overall fit.

Under GroupSplit on $\Delta$WorldPop (our primary setting), RMSE+CL consistently improves tail robustness. For example, SVR shows the largest gains: $R^2_{\text{all}}$ decreases slightly from 0.374 to 0.340, but $R^2_{\text{tail}}$ rises significantly from 0.283 to 0.483, and tail RMSE improves from 138.528 to 117.573. Random Forest shows similar patterns: tail $R^2$ increases from 0.573 to 0.595 with RMSE$_{\text{tail}}$ improving from 106.848 to 104.055. XGBoost also benefits: $R^2_{\text{tail}}$ improves from 0.563 to 0.577 and RMSE$_{\text{tail}}$ from 108.125 to 106.320. Tree ensembles like ETR, already strong baselines, show minimal response to calibration.

Strongest gains from CL-adjustment occur on tail metrics rather than overall metrics, confirming that the mixed objective acts as a tail-focused calibrator rather than global prediction rescaling—aligned with our objective of improving reliability on rare high-magnitude events.

In contrast, Table~\ref{tab:dwp_gs_avg_gain} shows CL-only frequently harms performance, suggesting contrastive objective alone is insufficient for preserving pointwise accuracy. Under GroupSplit, CL-only severely degrades Ridge and Elastic Net (both show negative overall $R^2$) and collapses SVR overall performance. This instability justifies using the RMSE-anchored mixed objective as default. Table~\ref{tab:dwp_gs_avg_gain} confirms this strategy remains stable and effective.

Overall, RS trends mirror GS patterns: RMSE+CL outperforms Base reliably, while CL-only is erratic. Since RS may allow temporal leakage within grid cells, we prioritize GS results as evidence of CL calibration effectiveness.
...
We investigate mechanisms underlying CL-adjustment improvements. Figure~\ref{fig:clvisualize} visualizes CL calibration effects. After adjustment, tail samples (upper-right) move closer to the diagonal, indicating improved prediction accuracy. However, some body samples shift away, suggesting a tail–body trade-off. This visualization confirms CL-adjustment effectiveness for tail prediction while revealing refinement opportunities.

With the mixed objective, overall performance is largely preserved while tail improves, showing that the RMSE component anchors predictions and prevents excessive body distortion.

\textbf{Summary:} CL calibration effectively improves tail predictions by reweighting model focus toward critical samples. The mixed objective maintains overall performance by balancing tail and body emphasis.

\subsection{Ablations (CL Calibration)}

We evaluate how mix-weight changes affect performance. Figure~\ref{fig:mixweightablation} shows the impact of varying mix weight on overall and tail metrics under GroupSplit on $\Delta$WorldPop. Results indicate a balanced mix around 5\% RMSE and 95% CL (our chosen weights) provides strong tail performance while maintaining reasonable overall accuracy.

We also note that some CL-adjusted models (MLP, Random Forest, Extra Trees) show limited tail improvements. This likely reflects that these base models are already quite strong, leaving limited room for calibrator enhancement.

\subsection{MoE: Negative Results}

We attempted Mixture-of-Experts as an alternative architecture for long-tail regression. However, experiments demonstrate that the two-expert MoE underperforms the baseline.

Table~\ref{tab:dwp_gs_moe_drop} shows that MoE (2-expert) under GroupSplit on $\Delta$WorldPop degrades performance versus XGBoost across all metrics:
...
Thus, we investigate MoE's underperformance through gating diagnostics in Figure~\ref{fig:gatingscatter}. While the gating network differentiates many samples, it fails to achieve sufficient accuracy for expert specialization. This causes suboptimal or incorrect expert assignments, degrading overall performance.

% =====================================================
\section{Discussion}

We evaluated machine learning models for predicting population and nighttime luminosity changes using CivitasGrid-TLP. The results demonstrate that RMSE-anchored contrastive calibration substantially improves tail performance without sacrificing overall accuracy. In contrast, the two-expert MoE unexpectedly underperformed. We also found that $\Delta$VIIRS is substantially more challenging to predict than $\Delta$WorldPop, likely reflecting increased noise from differencing. We discuss these findings in detail below.
...
The RMSE+CL mixed objective delivers consistent tail improvements. We attribute this to three mechanisms:
...
These findings indicate that simple binary MoE with soft/hard gating is ill-suited for this task. More robust approaches—uncertainty-aware gating, continuous mixtures, or threshold-aware routing—may better handle the ambiguity and high stakes of long-tail routing.
...
(1) \textit{VIIRS} prediction remains unsatisfactory. Future work should explore smoothed targets (e.g., three-year trends) to reduce differencing noise and improve signal.

(2) Our model predicts point estimates without uncertainty. Quantile regression or Bayesian approaches could enable uncertainty-aware prediction, improving decision support.
...
\textbf{Takeaway:} reliable tail gains require anchoring. For practitioners needing improved tail accuracy, RMSE-anchored contrastive calibration is a practical, lightweight add-on to strong tabular regressors. Naive tail/body MoE routing can collapse under feature–label ambiguity near quantile boundaries.

Future work should address VIIRS noise (via temporal smoothing or trend targets), incorporate uncertainty quantification (quantile or distributional regression), and explore uncertainty-aware or threshold-informed routing mechanisms to improve mixture-model stability in long-tail settings.


% =====================================================
\section*{Acknowledgments}
I am grateful to my advisor, Bryce Wang, for his guidance and feedback throughout this project.

I thank Lexie Guo and William Feng for their encouragement and helpful conversations. I also thank my parents, Yan Wang and Haijun Huang, for their continual support.

I thank Ben Chen for reviewing an early draft and providing valuable feedback.
...
CivitasGrid-TLP is publicly available at Zenodo (DOI\: 10.5281/zenodo.18144416)~\cite{huang_2026_18144416}.

The author thanks ChatGPT for assistance with language polishing and improving the clarity of the manuscript.
% ===================== REFERENCES ====================
% IEEE recommended:
% \bibliographystyle{IEEEtran}
% If you want to keep plainnat style exactly as before, you can,
% but IEEEtran style is more consistent with this template.
\bibliographystyle{IEEEtran}
